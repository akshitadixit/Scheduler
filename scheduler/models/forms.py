...
from scheduler.models.models import *
...
class NewCourseForm(FlaskForm):
    code = StringField('Project Code', validators=[InputRequired(), Length(min=6, max=9, message="Invalid Course Code")],
                       render_kw={"placeholder": "COMP 354"})
    title = StringField('Project Title', validators=[InputRequired(), Length(min=4, max=50, message="Invalid Course")],
                        render_kw={"placeholder": "Software Engineering"})
    lecturer = StringField('Manager', validators=[InputRequired(), Length(min=2, max=50, message="Invalid Lecturer")],
...
class NewExamForm(FlaskForm):
    course_code = StringField('Project code', validators=[InputRequired(), Length(min=4, max=50, message="Invalid Course"
                                                                                 )],
                              render_kw={"placeholder": "COMP 354"})
    exam_name   = StringField('Project Name', validators=[InputRequired(), Length(min=4, max=50, message="Invalid Exam Name")], render_kw={"placeholder":"Exam Name"})
    weighting = IntegerField('Weighting', validators=[InputRequired()],
    ...
    duration = IntegerField('Duration (In hours)', validators=[InputRequired(message = 'Please enter whole '
                                                                                            'number values'), ],
...
class NewAssignmentForm(FlaskForm):
    course_code = StringField('Project code', validators=[InputRequired(), Length(min=4, max=50, message="Invalid Course"
                              ...
                              render_kw={"placeholder": "COMP 354"})
    asg_name = StringField('Project Name', validators=[InputRequired(), Length(min=1, max=50,
                                                                                  message="Invalid Assignment Name")],
                           render_kw={"placeholder": "Assignment 1"})
    weighting = IntegerField('Project Weighting', validators=[InputRequired()],
                             render_kw={"placeholder": "100"})
