def main_func(filepath):
    ...
    for line in fhand:  # slide the file line by line
        ...
        for i in range(len(singleElement)):  # creating the single task element
            ...
            tasks['task' + str(singleElement[0])
                  ]['duration'] = singleElement[2]
            if(singleElement[3] != "\n"):
                tasks['task' + str(singleElement[0])
                      ]['dependencies'] = singleElement[3].strip().split(';')
            else:
        ...
        if('-1' in tasks[taskFW]['dependencies']):  # checks if it's the first task
            ...
            for k in tasks.keys():
                ...
                for dipendenza in tasks[k]['dependencies']:
                    ...
                    if(dipendenza != '-1' and len(tasks[k]['dependencies']) == 1):
                        tasks[k]['ES'] = int(
                            tasks['task' + dipendenza]['EF']) + 1
                        tasks[k]['EF'] = int(tasks[k]['ES']) + \
                            int(tasks[k]['duration']) - 1
                    elif(dipendenza != '-1'):  # if the task k has more dependency
                        if(int(tasks['task'+dipendenza]['EF']) > int(tasks[k]['ES'])):
                            tasks[k]['ES'] = int(
                                tasks['task' + dipendenza]['EF']) + 1
                            tasks[k]['EF'] = int(
                                tasks[k]['ES']) + int(tasks[k]['duration']) - 1
    ...
    for taskBW in bList:
        ...
        # slides all the dependency in a single task
        for dipendenza in tasks[taskBW]['dependencies']:
            if(dipendenza != '-1'):  # check if it's NOT the last task
                ...
                if(int(tasks['task' + dipendenza]['LF']) > int(tasks[taskBW]['LS'])):
                    tasks['task' +
                          dipendenza]['LF'] = int(tasks[taskBW]['LS']) - 1
                    tasks['task' + dipendenza]['LS'] = int(tasks['task' + dipendenza]['LF']) - int(
                        tasks['task' + dipendenza]['duration']) + 1
