var Intro = function () {
    ...
    this.$nav = $('.nav');
    this.w = $(window).width();
    this.coverW = this.w - this.$subtitle.width();
...
Intro.prototype = {
    ...
    introAnim: function () {
        ...
        introTween
            .staggerTo($(this.$strings[0]).find('.letter'), 0.1, {
                yPercent: -40,
                opacity: 1,
                ease: Back.easeOut
            }, 0.02666)
            ...
            .staggerTo($(this.$strings[1]).find('.letter'), 0.2, {
                yPercent: -40,
                opacity: 1,
                ease: Back.easeOut
            }, 0.025, '-=1.2')
            .staggerTo($(this.$strings[2]).find('.letter'), 0.2, {
                yPercent: -40,
                opacity: 1,
                ease: Back.easeOut
            }, 0.025, '-=0.9')
            .staggerTo($(this.$strings[3]).find('.letter'), 0.2, {
                yPercent: -40,
                opacity: 1,
                ease: Back.easeOut
            }, 0.025, '-=0.7')
            .to(this.$subtitle, 0.2, {
                opacity: 1,
                y: 0,
                ease: Back.easeOut
            }, '-=0.4')
